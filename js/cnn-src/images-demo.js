...
var visualize_activations = function(net, elt) {
  ...
  for(var i=0;i<N;i++) {
    ...
    var scale = 0; //chỉnh thông số hiển thị hình ảnh trong canvas

    if(L.layer_type==='softmax' || L.layer_type==='fc') scale = 2; // for softmax
...
var max_acc = parseFloat(65).toFixed(2);
var test_predict = function() {
  ...
  // lấy hình ảnh kiểm tra ngẫu nhiên
  for(num=0;num<2;num++) {
  ...
  var testAccvalue = parseFloat(testAccWindow.get_average()*100).toFixed(2);
  if($('#isSaveInfo').prop('checked') && testAccvalue > max_acc){

      save_network_to_jsonfile(dataset_name);
      save_information_when_get_maxacc(start_time);
      max_acc = testAccvalue;
      Save_Image_From_Canvas("lossgraph","loss-chart");
      Save_Image_From_Canvas("accuracy-chart","accuracy-chart");
      console.log(end_time);
  }
  console.log(start_time);
  $("#testset_acc").text('Test accuracy based on last 100 test images: ' + testAccvalue + "%");
}
...
var step = function(sample) {
  ...
  if(step_num % 100 === 0) {
    ...
    if(xa >= 0 && xw >= 0) { // if they are -1 it means not enough data was accumulated yet for estimates
      update_loss_chart(step_num, xLossWindow.get_average(), wLossWindow.get_average());
    if(trainAccWindow.get_average() >=0 && testAccWindow.get_average()>=0 && valAccWindow.get_average()>=0){
      update_accuracy_chart(step_num, trainAccWindow.get_average(), testAccWindow.get_average(), valAccWindow.get_average());
    }
  }
...
var toggle_start = function () {
  ...
  start_time = GetTimeString(d_start);
  start_fun();
}
...
var wloss_dataset = [];
var xloss_dataset = [];
var contruct_loss_chart = function(){
  var xloss_chart_data = {
      label:'Classification loss value',
      data: xloss_dataset,
      fill: false,
      lineTension: 0.01,
  ...
  var wloss_chart_data = {
      label:'Weight loss value',
      data: wloss_dataset,
      fill: false,
      lineTension: 0.01,
      backgroundColor: "rgba(85,187,85,0.4)",
      borderColor: "rgba(85,187,85,1)",
  };
  var loss_chart_data = {
    labels: loss_datalabels,
    datasets:[xloss_chart_data, wloss_chart_data]
  };
  var loss_chart_options = {
    title: { display: true, text: 'Training Loss' },
    showLine: true,
  };
  var loss_chart_canvas = document.getElementById("lossgraph");
  loss_chart = new Chart(loss_chart_canvas, {
    type: 'line',
    data: loss_chart_data,
    options: loss_chart_options
  });
}
var update_loss_chart = function(step_num_value, xLoss_value, wLoss_value){
  loss_datalabels.push(step_num_value);
  wloss_dataset.push(wLoss_value);
  xloss_dataset.push(xLoss_value);

  if(loss_datalabels.length > 10){
    loss_datalabels.shift();
    wloss_dataset.shift();
    xloss_dataset.shift();
  }
  loss_chart.update();
...
var accuracy_chart;
var accuracy_datalabels = [];
var train_acc_dataset = [];
var test_acc_dataset = [];
var vali_acc_dataset = [];
var contruct_accuracy_chart = function(){
  var vali_chart_dataset = {
    label:'Validation accuracy value',
    data: vali_acc_dataset,
    fill: false,
    lineTension: 0.1,
  ...
  var test_chart_dataset = {
    label:'Testing accuracy value',
    data: test_acc_dataset,
    fill: false,
    lineTension: 0.1,
    backgroundColor: "rgba(85,187,85,0.4)",
    borderColor: "rgba(85,187,85,1)",
  ...
  var train_chart_dataset = {
    label:'Training accuracy value',
    data: train_acc_dataset,
    fill: false,
    lineTension: 0.1,
    backgroundColor: "rgba(30,144,255,0.4)",
    borderColor: "rgba(30,144,255,1)"
  };
  var accuracy_chart_data = {
    labels: accuracy_datalabels,
    datasets:[test_chart_dataset, train_chart_dataset, vali_chart_dataset]
  };

  var accuracy_chart_options = {
    title: { display: true, text: 'Accuracy' },
    showLine: false,
  };

  var acc_chart_canvas = document.getElementById("accuracy-chart");

  accuracy_chart = new Chart(acc_chart_canvas, {
    type: 'line',
    data: accuracy_chart_data,
    options: accuracy_chart_options,
  });
}
var update_accuracy_chart = function(step_num_value, train_acc_value, test_acc_value, vali_acc_value){
  accuracy_datalabels.push(step_num_value);
  train_acc_dataset.push(train_acc_value);
  test_acc_dataset.push(test_acc_value);
  vali_acc_dataset.push(vali_acc_value);

  if(accuracy_datalabels.length > 10){
    accuracy_datalabels.shift();
    train_acc_dataset.shift();
    test_acc_dataset.shift();
    vali_acc_dataset.shift();
  }
  accuracy_chart.update();
}
...
var save_information_when_get_maxacc = function(starttime){
  ...
  var clasify_loss_train = parseFloat(xLossWindow.get_average()).toFixed(2)*100;
  var weight_loss = parseFloat(wLossWindow.get_average()).toFixed(2)*100;
  var validation_acc = parseFloat(valAccWindow.get_average()).toFixed(2)*100;
  var train_accuracy = parseFloat(trainAccWindow.get_average()).toFixed(2)*100;
  //json struct
